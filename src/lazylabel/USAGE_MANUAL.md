...
- Lost editability immediately when saved

**Polygon Segments (type: "Polygon")**  
- Hand-drawn using polygon mode
- Stored as vertex lists
- **Maintain editability after save/reload**
- Can be edited in Edit Mode (`R`)
...
| Action | Primary Key | Secondary Key | Description |
|--------|-------------|---------------|-------------|
| Merge Selected | `M` | - | Combine multiple segments |
...
- **SAM 2.1** - Enhanced accuracy, requires additional installation
...
- **Output:** NPZ (primary), JSON (metadata)
