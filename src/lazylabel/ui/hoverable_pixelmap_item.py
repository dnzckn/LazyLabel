...
class HoverablePixmapItem(QGraphicsPixmapItem):
    ...
    def set_pixmaps(self, default_pixmap, hover_pixmap):
        ...
        logger.debug(f"HoverablePixmapItem.set_pixmaps: default={default_pixmap}, hover={hover_pixmap}")

    def set_segment_info(self, segment_id, main_window):
        from lazylabel.utils.logger import logger
        self.segment_id = segment_id
        self.main_window = main_window
        logger.debug(f"HoverablePixmapItem.set_segment_info: segment_id={segment_id}, main_window={main_window is not None}")

    def hoverEnterEvent(self, event):
        from lazylabel.utils.logger import logger
        logger.debug(f"HoverablePixmapItem.hoverEnterEvent: segment_id={self.segment_id}, main_window={self.main_window is not None}")
    ...
    def hoverLeaveEvent(self, event):
        from lazylabel.utils.logger import logger
        logger.debug(f"HoverablePixmapItem.hoverLeaveEvent: segment_id={self.segment_id}, main_window={self.main_window is not None}")
        self.setPixmap(self.default_pixmap)
