...
def main_window_with_multi_view(qtbot, mock_sam_models_list):
    ...
    with (
        patch("lazylabel.core.model_manager.ModelManager.initialize_default_model") as mock_init,
        patch("lazylabel.core.model_manager.ModelManager.get_available_models") as mock_get_models,
        patch("lazylabel.core.model_manager.ModelManager.is_model_available") as mock_is_available,
...
@patch("lazylabel.ui.main_window.SamModel")
def test_multi_view_model_initialization_sam1(mock_sam_model_class, main_window_with_multi_view):
    ...
    assert mock_sam_model_class.call_count == 2
    window._show_success_notification.assert_called_once()


@patch("lazylabel.ui.main_window.Sam2Model")
def test_multi_view_model_initialization_sam2(mock_sam2_model_class, main_window_with_multi_view):
    ...
    assert len(window.multi_view_models) == 2
    assert mock_sam2_model_class.call_count == 2
    window._show_success_notification.assert_called_once()
...
def test_multi_view_batch_loading(main_window_with_multi_view):
    ...
    window.control_panel.border_crop_widget.disable_thresholding_for_multi_view = MagicMock()
...
def test_multi_view_save_batch(main_window_with_multi_view):
    ...
    window.file_manager.save_segments_to_npz = MagicMock()
    
    # Test batch save
    window._save_multi_view_batch()
    
    # Verify both images were saved
    assert window.file_manager.save_segments_to_npz.call_count == 2
...
def test_multi_view_mouse_event_delegation(main_window_with_multi_view):
    ...
    window._handle_multi_view_ai_click.assert_called_once_with(pos, 0, mock_event)
